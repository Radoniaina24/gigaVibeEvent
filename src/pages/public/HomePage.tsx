...
function Container({ children }: { children: React.ReactNode }) {
  return <div className="mx-auto max-w-7xl px-4 md:px-2">{children}</div>;
}
      ...
      <section aria-label="Explorer par catégorie" className="bleed">
        <Container>
          {categoriesQuery.isPending ? (
            ...
            <ul className="no-scrollbar -mx-4 flex gap-2 overflow-x-auto px-4 pb-1 md:-mx-2 md:flex-wrap md:px-2">
      ...
      {!eventsQuery.isPending && !eventsQuery.isError && lastChance.length > 0 && (
        <section aria-label="Dernière chance" className="bleed">
          <Container>
            ...
            />
            <div className="no-scrollbar -mx-4 mt-5 flex snap-x snap-mandatory gap-4 overflow-x-auto px-4 pb-2 md:-mx-2 md:px-2">
              {lastChance.map((e) => (
